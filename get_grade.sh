...
OUTPUT=$(echo "$PROJECT_PAGE" | sed -n '/data-status/,$p' | sed '/project-summary-item/,$d' | sed 's/<[^<>]*>//g' | sed -r '/^\s*$/d')
STATUS=$(echo "$OUTPUT" | grep -E "(fail|success)")
GRADE=$(echo "$OUTPUT" | grep -E "[0-9]")

if [ -z "$STATUS" ];
then STATUS="unregistered"
fi
